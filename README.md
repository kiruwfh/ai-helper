...
- Tries to read text from images via OCR and appends short, human-readable image summaries to the exported text.
...
- (Optional but recommended) [Tesseract OCR](https://github.com/tesseract-ocr/tesseract) installed on the host machine to enable automatic extraction of text from images.
...
2. `page.txt` – the plain text extracted from the page for quick reference or sharing with other AI tools. The file now contains a section that lists every image, the recognized text (if any), and a short description so that downstream AI models see the same context you do.
...
- If Tesseract OCR is not installed the bot still works, but it will warn you in chat and skip text recognition inside images.
